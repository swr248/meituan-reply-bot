...
class VncAuthTests(unittest.TestCase):
    def setUp(self):
        ...
        cookie_capture._cfg = {'server': {'auth_token': 'shop2-token', 'legacy_auth_tokens': ['old-shop2-token']}}

    def tearDown(self):
        cookie_capture._cfg = self.original_config

    def test_token_validation_accepts_current_and_legacy_tokens(self):
        self.assertTrue(cookie_capture._token_is_valid('shop2-token'))
        self.assertTrue(cookie_capture._token_is_valid('old-shop2-token'))
